...
const generateSectionHtml = (sec: { title: string; icon: string }, i: number, topic: string, categoryId?: string): string => {
  ...
  const isDesign = categoryId === 'design';
  ...
  return `<div id="module-${i + 1}" class="bg-glass hover:bg-glass-hover border border-black/20 dark:border-white/10 rounded-3xl p-6 md:p-8 transition-all duration-300 shadow-xl relative overflow-hidden group hover:border-primaryLight/40">
    ...
    <div class="relative z-10">
      <div class="flex items-center justify-between gap-4 mb-6">
        ...
        ${!isDesign ? `<a href="${playUrl}" class="hidden sm:inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-main text-white font-bold text-xs shadow-lg shadow-primary/20 hover:scale-105 transition-all">Practice Problem ${i + 1} ⚡</a>` : ''}
      </div>
      <div class="prose dark:prose-invert max-w-none text-textMuted text-lg leading-relaxed mb-8">
        <p>In this module, we dissect <strong>${cleanTitle}</strong>. Understanding this concept is fundamental to mastering ${topic}.</p>
      ...
      <div class="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-6">
        ...
        <div class="flex items-center gap-3">
          ${!isDesign ? `<a href="${playUrl}" class="inline-flex sm:hidden items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-main text-white font-bold text-xs">Practice ⚡</a>` : ''}
...
export const generateRichContent = (topic: string, categoryId: string): string => {
  const sectionsHtml = MODULE_SECTIONS.map((sec, i) => generateSectionHtml(sec, i, topic, categoryId)).join('');
